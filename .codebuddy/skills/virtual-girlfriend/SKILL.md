...
3. **零复制粘贴**
   - chat.py 和 server.py 是同一核心的两个「门面」，共享功能必须在中间层实现
   - 已抽模块: agent_core.py（Agent 循环）、session.py（会话管理）、llm_client.py（通信）
   - 发现两边有重复逻辑 → 立即抽模块 → 不留技术债

4. **职责分层（从底到顶）**
   ```
   llm_client.py        ← 只负责"怎么跟 Ollama 说话"
   experience_store.py  ← 向量存储
   tools/              ← 工具集
   agent_core.py       ← Agent 循环 + system prompt
   session.py          ← 会话管理（加载/保存/压缩/持久化）
   extraction.py       ← 事实/经历提取
   chat.py / server.py ← 终端版 / Web 版（最外层，只做 IO）
   ```

3. **重构时机**
   - 意识到可以抽模块 → 立即动手，不堆积
   - 重构时先说明"改哪里/为什么/效果"，再执行
   - 重构后立即更新本文件
